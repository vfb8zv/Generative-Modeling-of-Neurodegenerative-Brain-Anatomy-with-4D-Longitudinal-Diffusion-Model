...
class DataLoading(Dataset):
    def __init__(self, path, test_flag=False):
        ...
        self.files = self.files_df2 + self.files_df1 
    ...
    def __getitem__(self, idx):
        ...
        im = torch.stack([std_img(im[__]) for __ in range(len(im))], dim=0)
...
def get_data(path='./dataset/', b=1, test_flag=False, large_def_split=0.7):
